...
export default function DashboardPage() {
  ...
  // Show cockfight calendar only for admin roles
  const showCockfightCalendar = isAdmin;
      ...
      <Layout>
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            {showPermits && permitCategories.length > 0 && (
              <div className="flex flex-wrap gap-2">
                <button onClick={() => setSelectedCategory('')} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedCategory === '' ? 'bg-slate-800 text-white shadow' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}>All Permits</button>
          ...
          {/* ── RIGHTS & RENTALS MANAGER: Full R&R dashboard ── */}
          {isRRManager && !isAdmin && (
            <>
              {/* R&R Summary Cards */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-white rounded-xl border border-slate-200 p-5">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      <p className="text-xl font-bold text-slate-800">{formatCurrency(data.rightsAndRentals.rightsCollected)}</p>
                      <p className="text-xs text-slate-500">Rights Collected</p>
                    </div>
                  </div>
                </div>
                <div className="bg-white rounded-xl border border-slate-200 p-5">
                  <div className="flex items-center gap-3 mb-1">
                    <div className="h-10 w-10 rounded-lg bg-orange-50 flex items-center justify-center"><svg className="h-5 w-5 text-orange-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg></div>
                    <div>
                      <p className="text-xl font-bold text-slate-800">{formatCurrency(data.rightsAndRentals.rentalCollected)}</p>
                      <p className="text-xs text-slate-500">Rental Collected</p>
                    </div>
                  </div>
                </div>
                <div className="bg-white rounded-xl border border-emerald-200 p-5 bg-emerald-50">
                  <div className="flex items-center gap-3 mb-1">
                    <div className="h-10 w-10 rounded-lg bg-emerald-100 flex items-center justify-center"><svg className="h-5 w-5 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></div>
                    <div>
                      <p className="text-xl font-bold text-emerald-700">{formatCurrency(data.rightsAndRentals.rightsCollected + data.rightsAndRentals.rentalCollected)}</p>
          ...
          {/* ── TRAFFIC OFFICER: Citations focus ── */}
          ...
          {/* ── PERMIT ROLES (Assessor, Approver, App Creator): Permits focus ── */}
          {showPermits && !isAdmin && (
            <>
                ...
                {permitCards.map((card) => (
                  <div key={card.label} onClick={() => { const p = new URLSearchParams(); if (card.filter) p.set('filter', card.filter); if (selectedCategory) p.set('permitCategory', selectedCategory); router.push(`/applications${p.toString() ? '?' + p.toString() : ''}`); }} className={`group bg-white rounded-xl border border-slate-200 p-4 cursor-pointer hover:shadow-md ${card.hoverBorder} transition-all active:scale-[0.98]`}>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <svg className={`h-4 w-4 ${card.arrowColor} opacity-0 group-hover:opacity-100 transition-all`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                    </div>
                    <p className="text-2xl font-bold text-slate-800">{card.value}</p>
                    <p className="text-xs text-slate-500 mt-0.5">{card.label}</p>
                  </div>
          ...
          {/* ── ADMIN / SUPERADMIN: Full dashboard ── */}
          {isAdmin && (
            <>
              {/* Permit Status Cards */}
              <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-5">
                  ...
                  <div className="mt-1"><div className="h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-emerald-500 rounded-full" style={{ width: `${data.citations.total > 0 ? (data.citations.paid / data.citations.total) * 100 : 0}%` }}></div></div></div>
                </div>
                <div className="bg-white rounded-xl border border-slate-200 p-5">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="h-10 w-10 rounded-lg bg-sky-50 flex items-center justify-center"><svg className="h-5 w-5 text-sky-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-5">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="h-10 w-10 rounded-lg bg-violet-50 flex items-center justify-center"><svg className="h-5 w-5 text-violet-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg></div>
                    <div><p className="text-2xl font-bold text-slate-800">{data.entities}</p><p className="text-xs text-slate-500">Registered Entities</p></div>
                  </div>
                </div>
              </div>
              ...
              {/* Rights & Rentals Summary */}
              <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="h-8 w-8 rounded-lg bg-orange-50 flex items-center justify-center"><svg className="h-4 w-4 text-orange-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg></div>
                    <h3 className="text-sm font-semibold text-slate-700">Rights &amp; Rentals</h3>
                  </div>
                  <Link href="/admin/rights-and-rentals" className="text-xs font-medium text-orange-600 hover:text-orange-700">View module</Link>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 p-5 border-b border-slate-100">
                  {[
                    { label: 'Lessees', value: data.rightsAndRentals.totalLessees },
                    { label: 'Properties', value: data.rightsAndRentals.totalProperties },
                    { label: 'Active Contracts', value: data.rightsAndRentals.activeContracts, color: 'text-emerald-600' },
                    { label: 'Total Contracts', value: data.rightsAndRentals.totalContracts },
                  ].map((item) => (
                  ...
                  <div className="text-center col-span-2 sm:col-span-1">
                    <p className="text-xl font-bold text-slate-800">{formatCurrency(data.rightsAndRentals.rightsCollected + data.rightsAndRentals.rentalCollected)}</p>
                    <p className="text-xs text-slate-500 mt-0.5">Total Collected</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-slate-100">
                  <div className="p-5">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Collections Breakdown</p>
                    <div className="space-y-3">
                      <div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-sky-500"></span><span className="text-sm text-slate-600">Rights Payments</span></div><span className="text-sm font-semibold text-slate-800">{formatCurrency(data.rightsAndRentals.rightsCollected)}</span></div>
                      <div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-orange-400"></span><span className="text-sm text-slate-600">Rental Payments</span></div><span className="text-sm font-semibold text-slate-800">{formatCurrency(data.rightsAndRentals.rentalCollected)}</span></div>
                      <div className="pt-2 border-t border-slate-100 flex items-center justify-between"><span className="text-sm font-medium text-slate-700">Total Collected</span><span className="text-sm font-bold text-emerald-600">{formatCurrency(data.rightsAndRentals.rightsCollected + data.rightsAndRentals.rentalCollected)}</span></div>
                    </div>
                  </div>
                  <div className="p-5">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Recent Payments</p>
                    {data.rightsAndRentals.recentPayments.length > 0 ? (
                      <div className="space-y-2">
                        {data.rightsAndRentals.recentPayments.map((payment, idx) => (
                          <div key={idx} className="flex items-center justify-between text-xs">
                            <div className="min-w-0 flex-1"><p className="font-medium text-slate-700 truncate">{payment.lessee_name}</p><p className="text-slate-400 truncate">{payment.property_name}</p></div>
                            <div className="flex flex-col items-end ml-3 flex-shrink-0">
                              <span className="font-semibold text-slate-800">{formatCurrency(payment.amount_paid)}</span>
                              <span className={`mt-0.5 px-1.5 py-0.5 rounded-full text-xs font-medium ${payment.payment_type === 'rights' ? 'bg-sky-100 text-sky-700' : 'bg-orange-100 text-orange-700'}`}>{payment.payment_type}</span>
                            </div>
                          </div>
                        ))}
                      </div>
