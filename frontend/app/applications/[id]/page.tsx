                ...
                <div className="flex gap-2 overflow-x-auto pb-2 -mx-2 px-2 scrollbar-hide touch-scroll">
                  ...
                  {(
                    <>
                      ...
                      {/* Reports Dropdown - Fixed position below button */}
                      {showReportsDropdown && (
                        <>
                          ...
                          >
                            ...
                            {(application.status === 'Assessed' || application.status === 'Pending Approval' || application.status === 'Approved' || application.status === 'Paid' || application.status === 'Issued' || application.status === 'Released') && (
                              application.attribute_name && application.attribute_name.trim().toUpperCase() === 'DISCO' ? (
                                <button
                                  onClick={() => {
                                    const token = localStorage.getItem('token') || '';
                                    const encodedToken = token ? `&token=${encodeURIComponent(token)}` : '';
                            ...
                            {(application.status === 'Approved' || application.status === 'Paid' || application.status === 'Issued' || application.status === 'Released') && (
                              <button
                                onClick={() => {
                                  const token = localStorage.getItem('token') || '';
                                  const encodedToken = token ? `&token=${encodeURIComponent(token)}` : '';
                                  const url = `/business-permit.html?id=${application.application_id}${encodedToken}`;
                                  openReportModal(url, 'Business Permit');
                                }}
            ...
            <div className="bg-white rounded-lg shadow-2xl w-full h-full max-w-5xl max-h-[90vh] flex flex-col">
              ...
              <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setReportUrl(reportBaseUrl + (reportBaseUrl.includes('?') ? '&' : '?') + '_v=' + Date.now())}
