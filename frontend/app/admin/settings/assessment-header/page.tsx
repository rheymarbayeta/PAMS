...
const DEFAULT_LINES: Record<string, string> = {
  assessment_header_line_1: 'REPUBLIC OF THE PHILIPPINES',
  assessment_header_line_2: 'MUNICIPALITY OF DALAGUETE',
  assessment_header_line_3: 'PROVINCE OF CEBU',
  assessment_header_line_4: 'OFFICE OF THE MUNICIPAL ASSESSOR',
  assessment_header_line_5: 'Tel. # 520 4141 loc. 315',
};

interface FormData {
  assessment_header_logo_left: string;
  assessment_header_logo_right: string;
  assessment_header_line_1: string;
  assessment_header_line_2: string;
  assessment_header_line_3: string;
  assessment_header_line_4: string;
  assessment_header_line_5: string;
  assessment_header_font_size: string;
  assessment_header_logo_size: string;
}

export default function AssessmentHeaderSettingsPage() {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [formData, setFormData] = useState<FormData>({
    assessment_header_logo_left: '',
    assessment_header_logo_right: '',
    assessment_header_line_1: DEFAULT_LINES.assessment_header_line_1,
    assessment_header_line_2: DEFAULT_LINES.assessment_header_line_2,
    assessment_header_line_3: DEFAULT_LINES.assessment_header_line_3,
    assessment_header_line_4: DEFAULT_LINES.assessment_header_line_4,
    assessment_header_line_5: DEFAULT_LINES.assessment_header_line_5,
    assessment_header_font_size: '11',
    assessment_header_logo_size: '50',
  });
  ...
  useEffect(() => {
    api.get('/api/settings').then(r => {
      const d = r.data;
      setFormData({
        assessment_header_logo_left:  d.assessment_header_logo_left?.value  || '',
        assessment_header_logo_right: d.assessment_header_logo_right?.value || '',
        assessment_header_line_1: d.assessment_header_line_1?.value ?? DEFAULT_LINES.assessment_header_line_1,
        assessment_header_line_2: d.assessment_header_line_2?.value ?? DEFAULT_LINES.assessment_header_line_2,
        assessment_header_line_3: d.assessment_header_line_3?.value ?? DEFAULT_LINES.assessment_header_line_3,
        assessment_header_line_4: d.assessment_header_line_4?.value ?? DEFAULT_LINES.assessment_header_line_4,
        assessment_header_line_5: d.assessment_header_line_5?.value ?? DEFAULT_LINES.assessment_header_line_5,
        assessment_header_font_size: d.assessment_header_font_size?.value ?? '11',
        assessment_header_logo_size: d.assessment_header_logo_size?.value ?? '50',
      });
  ...
  const getLogoSrc = (key: 'assessment_header_logo_left' | 'assessment_header_logo_right') => {
  ...
  const leftLogoSrc  = getLogoSrc('assessment_header_logo_left');
  const rightLogoSrc = getLogoSrc('assessment_header_logo_right');
      ...
      <Layout>
        <div className="px-4 py-8 sm:px-6 lg:px-8 max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span className="font-medium text-gray-900">Assessment Header</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Assessment Header Settings</h1>
              <p className="text-sm text-gray-500">Customize the logo(s) and header text on assessment documents</p>
            </div>
          </div>
              ...
              <div className="flex items-center gap-4">
                {leftLogoSrc && (
                  ...
                  <img src={leftLogoSrc} alt="Left logo"
                    style={{ height: `${Math.max(20, Math.min(120, Number(formData.assessment_header_logo_size) || 50))}px`, width: 'auto', objectFit: 'contain', flexShrink: 0 }}
                  />
                )}
                {(() => {
                  const base = Math.max(6, Math.min(32, Number(formData.assessment_header_font_size) || 11));
                  const line4Size = Math.round(base * 1.45);
                  return (
                    <div className="text-center leading-snug">
                      {formData.assessment_header_line_1 && (
                        <div style={{ fontSize: base, fontWeight: 'bold' }}>{formData.assessment_header_line_1}</div>
                      )}
                      {formData.assessment_header_line_2 && (
                        <div style={{ fontSize: base, fontWeight: 'bold' }}>{formData.assessment_header_line_2}</div>
                      )}
                      {formData.assessment_header_line_3 && (
                        <div style={{ fontSize: base, fontWeight: 'bold' }}>{formData.assessment_header_line_3}</div>
                      )}
                      {formData.assessment_header_line_4 && (
                        <div style={{ fontSize: line4Size, fontWeight: 'bold', marginTop: 2 }}>{formData.assessment_header_line_4}</div>
                      )}
                      {formData.assessment_header_line_5 && (
                        <div style={{ fontSize: base, fontWeight: 'normal', marginTop: 2 }}>{formData.assessment_header_line_5}</div>
                      )}
                    </div>
                  );
                })()}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-lg shadow-gray-200/50 p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <LogoUploadCard
                label="Left Logo"
                settingKey="assessment_header_logo_left"
                savedUrl={formData.assessment_header_logo_left}
                previewDataUrl={logoPreview['assessment_header_logo_left'] || ''}
                uploading={uploading['assessment_header_logo_left'] || false}
                onFileSelect={handleFileSelect}
                onClear={handleClearLogo}
              />
              <LogoUploadCard
                label="Right Logo (optional)"
                settingKey="assessment_header_logo_right"
                savedUrl={formData.assessment_header_logo_right}
                previewDataUrl={logoPreview['assessment_header_logo_right'] || ''}
                uploading={uploading['assessment_header_logo_right'] || false}
                onFileSelect={handleFileSelect}
                onClear={handleClearLogo}
              />
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-lg shadow-gray-200/50 p-6">
            ...
            <div className="mb-5 p-4 bg-gray-50 rounded-xl border border-gray-200">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Base Font Size
                <span className="ml-2 text-indigo-600 font-semibold">{formData.assessment_header_font_size} px</span>
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  max={20}
                  step={1}
                  value={formData.assessment_header_font_size}
                  onChange={(e) => setFormData(prev => ({ ...prev, assessment_header_font_size: e.target.value }))}
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setFormData(prev => ({ ...prev, assessment_header_font_size: String(v) }));
                  }}
            ...
            <div className="mb-5 p-4 bg-gray-50 rounded-xl border border-gray-200">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Logo Height
                <span className="ml-2 text-indigo-600 font-semibold">{formData.assessment_header_logo_size} px</span>
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  max={120}
                  step={2}
                  value={formData.assessment_header_logo_size}
                  onChange={(e) => setFormData(prev => ({ ...prev, assessment_header_logo_size: e.target.value }))}
                  className="flex-1 h-2 accent-indigo-600"
                />
                <input
                  type="number"
                  ...
                  onChange={(e) => {
                    ...
                    setFormData(prev => ({ ...prev, assessment_header_logo_size: String(v) }));
                  }}
                  className="w-16 border border-gray-200 rounded-lg px-2 py-1.5 text-center text-sm font-medium text-gray-900 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none"
                />
              </div>
            ...
            <div className="space-y-3">
              {[1, 2, 3, 4, 5].map(n => {
                const key = `assessment_header_line_${n}` as keyof FormData;
                return (
                  <div key={n}>
