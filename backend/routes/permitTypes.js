...
router.get('/', async (req, res) => {
  try {
    const [permitTypes] = await pool.execute(
      `SELECT pt.*, a.attribute_name, a.attribute_id
...
router.post('/', authorize('SuperAdmin', 'Admin'), async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.execute(
      'INSERT INTO Permit_Types (permit_type_name, attribute_id, description, is_active) VALUES (?, ?, ?, ?)',
      [permit_type_name, attribute_id ? parseInt(attribute_id) : null, description || null, is_active !== undefined ? is_active : true]
    );
      ...
      for (const fee of fees) {
        if (fee.fee_id && fee.default_amount !== undefined) {
          await connection.execute(
            'INSERT INTO Permit_Type_Fees (permit_type_id, fee_id, default_amount, is_required) VALUES (?, ?, ?, ?)',
            [
              permitTypeId,
              fee.fee_id,
              parseFloat(fee.default_amount),
    ...
    res.status(201).json({
      ...
      permit_type_name,
      attribute_id: attribute_id ? parseInt(attribute_id) : null,
      description,
...
router.put('/:id', authorize('SuperAdmin', 'Admin'), async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.execute(
      ...
      [permit_type_name, attribute_id ? parseInt(attribute_id) : null, description || null, is_active !== undefined ? is_active : true, permitTypeId]
    );
